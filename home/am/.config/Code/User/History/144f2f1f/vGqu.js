...
function generarNodo(tipo, root=$formNode) {
...
function mostrarForm(id=null, mostrarSoloFicha=false) {
    
    limpiar('container');
    
    if (mostrarSoloFicha) {
        document.getElementById("container").append($select_ficha);
        return;
    }
    //mostrar formulario
    let form = formTemplate.content.firstElementChild.cloneNode();
    const footer = $form.querySelector(".card-footer").cloneNode();
    //BOTON CANCELAR
    let cancelarBtn = $form.querySelector(".cancelar-btn").cloneNode(true);
    cancelar(cancelarBtn);
    //BOTON GUARDAR
    let guardarBtn = $form.querySelector(".guardar-btn").cloneNode(true);
