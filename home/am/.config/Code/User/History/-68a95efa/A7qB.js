...
export class SCSInputPrice extends SCSInput {
    ...
    this.$input.oninput = function(e) {
      let val = e.target.value;
			let parsed = this.parse_price(val);
      e.target.value = !isNaN(parsed) ? this.format_price(parsed) : val; 
    }.bind(this);
  ...
  parse_price(val) {
    // let par = parseFloat(val.trim().replace('.', '').replace(',',''));
    let par = val.trim().replace('.', '').replace(',','');
    console.log("🚀 ~ SCSInputPrice ~ parse_price ~ par:", par)
		return par;
  }

  format_price(val) {
		console.log("🚀 ~ SCSInputPrice ~ format_price ~ val:", val)
		// let v1 = (val/100).toFixed(2).replace(/^(\d)(?=\.)/, '0$1');
		// let v2 = val.toFixed(2).replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1,');
		// let v2 = (val/100).toFixed(2).replace('.',',').replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');
		// let v2 = val.replace('.',',').replace(/(\d)(?=(\d{3})+(?!\d))/g, '$1.');
		let v2 = val.replace('.',',').replace(/(\d{1,3})(?=(\d{3})+(?!\d))/g, '$1.');
//(\d{1,3})(?=(\d{3})+(?!\d)) commas
		console.log("🚀 ~ SCSInputPrice ~ format_price ~ v2:", v2)
		return v2;
  }
