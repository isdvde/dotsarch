...
export class SCSFormRequisicion extends SCSForm {
  constructor(opts) {
    super(opts);
    this.data_to_edit = this.edit_data.requisicion;
  }
  ...
  add_detalles() {
    this.$detalles = SCSDetalleFactory(this.data_to_edit.tipo).create();
	...
	async load_to_edit(uuid) {
      ...
      this.add_detalles();
			...
			for(let [key, val] of Object.entries(this.data_to_edit)) {
