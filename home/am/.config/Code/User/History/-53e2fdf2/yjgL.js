...
class SCSDetalleBienes extends SCSDetalle {
  constructor(opts){
    ...
    this.$select = new SCSInputFactory('select_search').create({
  ...
  async create_detalle(art_id, opts) {
    ...
    let $cant =        new SCSInputFactory().create({ type: "number", size: 2, placeholder: "Cant" });
    let $articulo =    new SCSInputFactory().create({ type: "text", size: 3, placeholder: "Articulo" });
    let $codigo =      new SCSInputFactory().create({ type: "text", size: 3, placeholder: "Codigo" });
    let $um =          new SCSInputFactory().create({ type: "text", size: 3, placeholder: "Unidad Medida" });
    let $articulo_id = new SCSInputFactory('hidden').create({type: "hidden"});
    let $um_id =       new SCSInputFactory('hidden').create({type: "hidden"});
    let $linea_id =    new SCSInputFactory('hidden').create({type: "hidden"});
    let $uuid =        new SCSInputFactory('hidden').create({type: "hidden"});
