...
export class SCSForm extends HTMLElement {
	...
	async load_to_edit(uuid) {
		let inputs = {
			...
			"select-multiple": async function(inp, uuid){
			 await inp.closest('scs-select-search').init_value(uuid);
			},
		...
		if(this.edit_data) {
			console.log("🚀 ~ SCSForm ~ load_to_edit ~ edit_data:", this.edit_data)
			for(let [key, val] of Object.entries(this.edit_data)) {
				...
				if(inp && val){
					console.log("🚀 ~ SCSForm ~ load_to_edit ~ inp:", inp)
					console.log("🚀 ~ SCSForm ~ load_to_edit ~ inp.type:", inp.type)
					// if(inp.type === 'date') {
					// 	inp.value = val.slice(0, 10);
					// } else {
					// 	inp.value = val;
					// }
					try {
						inputs[inp.type](inp, val=val, uuid=uuid) 
					} catch {
	...
	async submit(e) {
		...
		try {
			if (this.data) {
				...
				console.log("🚀 ~ SCSForm ~ submit ~ options:", options)
