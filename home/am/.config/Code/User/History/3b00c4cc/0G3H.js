...
export class SCSRequisicionBase extends SCSBaseModule {
  ...
  async render_table() {
    ...
    if(!this.dependencia_solicitante){
      this.$root.append(new SCSNotification({data: ["Erro al conectar con el servidor, contactese con soporte"]}));
      this.$table.$button_container.remove();
    }
  }
