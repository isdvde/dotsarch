...
export class SCSDetalleBienes extends SCSDetalle {
  constructor(opts){
    super(opts);
    this.url_articulo =  'api/compras/articulo';

    this.$select = SCSInputFactory('select_search').create({
  ...
  async add_detalle(opts){
    ...
    if(! this.check_value(id, $values)) return

    let $detalle = (await this.create_detalle(id, opts));
  ...
  async create_detalle(art_id, opts) {
    ...
    let $cant =        SCSInputFactory().create({ type: "number", size: 2, placeholder: "Cant" });
    let $articulo =    SCSInputFactory().create({ type: "text", size: 3, placeholder: "Articulo" });
    let $codigo =      SCSInputFactory().create({ type: "text", size: 3, placeholder: "Codigo" });
    let $um =          SCSInputFactory().create({ type: "text", size: 3, placeholder: "Unidad Medida" });
    let $articulo_id = SCSInputFactory('hidden').create({type: "hidden"});
    let $um_id =       SCSInputFactory('hidden').create({type: "hidden"});
    let $linea_id =    SCSInputFactory('hidden').create({type: "hidden"});
...
customElements.define('scs-detalle-bienes', SCSDetalleBienes);