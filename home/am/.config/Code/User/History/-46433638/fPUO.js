export class Validator {
  ...
  validate() {
    ...
    this.isValid();
    if(this.result.message.length !== 0){
  ...
  isValid() {
    ...
    if (this.validity.rangeOverflow)
      this.result.message = `Ingrese un valor no mayor a ${this.$node.getAttribute("max")}`;

    // If a number field is below the min
    if (this.validity.rangeUnderflow)
      this.result.message = `Ingrese un valor no menor a ${this.$node.getAttribute("min")}`;
