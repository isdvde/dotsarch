...
export class TableComponent extends HTMLElement {
		...
		this.innerHTML = `
			<div>
				<div class="card-header d-flex gap-6 justify-content-between align-items-center">
					<input type="text" class="form-control w-75 mx-3" name="search" placeholder="Buscar">
						...
						<div class="table-responsive">
							<table class="table">
								<thead class="bg-primary-gradient">
										<tr>
												@forelse ($encabezados as $index => $encabezado)
														<th class="text-white text-center font-bold tracking-wider" {{ isset($anchos[$index]) ? "width=$anchos[$index]" : "" }} > {{ $encabezado }} </th>
												@empty
														<th class="text-white text-center font-bold tracking-wider"></th>
												@endforelse
										</tr>
								</thead>
	...
	action_buttons() {
		...
		for (let [i, $tr] of $trs.entries()) {
			let td = $td.cloneNode();
			let $buttons = new DOMParser().parseFromString(this.action_buttons_template, 'text/html').querySelector('body').firstChild;
	...
	update_pagination() {
		this.$pagination.update(this.data.pagination);
	}
...
customElements.define('z-table', TableComponent);