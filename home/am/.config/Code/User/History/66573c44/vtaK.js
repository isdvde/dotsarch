...
export class TableComponent extends HTMLElement {
	...
	action_buttons($tr, n) {
		...
		// for (let [i, $tr] of $trs.entries()) {
			...
			$edit.setAttribute('action-id', profesores[n].uuid);
			$delete.setAttribute('action-id', profesores[n].uuid);
	...
	async delete_item(e){
		console.log(e);
		let uuid = e.target.getAttribute('action-id');
		try {
			if(uuid) {
				let options = {
					method: "DELETE",
