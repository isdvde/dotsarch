export function $(sel) {
	return document.querySelector(sel);
}

export function $create(el) {
	return document.createElement(el);
}

export function sleep(ms){
	return new Promise(resolve => setTimeout(resolve, ms));
};

export function get_current_date() {
	let date = new Date();
	return `${date.getFullYear()}-${('0' + date.getMonth()+1).slice(-2)}-${('0' + date.getDate()).slice(-2)}`
}

export function get_quarter(val) {
	let d = new Date(val);
	return String(Math.floor((d.getMonth() + 3) / 3));
}

export const $qs = (parent, selector) => parent.querySelector(selector);
export const $qsa = (parent, selector) => parent.querySelectorAll(selector);

export const crudder = (url, opts={}) => {
	return {
		get: async () => await axios(url).then(res => res.data),
		create: async (data) => {
			return await axios({
				...opts,
				method: 'POST',
				url: url,
				data: data,
			}).then(res => res.data).catch(res => res.response.data);
		},
		update: async (data) => {
			return await axios({
				...opts,
				method: 'PUT',
				url: url,
				data: data,
			}).then(res => res.data).catch(res => res.response.data);
		},
		delete: async () => {
			return await axios({
				...opts,
				method: "DELETE",
				url: url,
			}).then((res) => res.data);
		}
	}
}

export const get_token = (parent) => {
	return $qs(parent, 'input[name="_token"]').value
}

export const alerter = msg => {
	return {
		alert: () => window.alert(msg),
		confirm: () => window.confirm(msg)
	}
}

export const clean = el => el.innerHTML = '';