import { $create, $qs } from "../libs/utils";
...
export class SCSDetalleServicios extends SCSDetalle {
  ...
  create_detalle(opts) {
    ...
    let $descripcion = SCSInputFactory('textarea').create({
      type: "text", title: "Descripcion", size: 12, height: 3, placeholder: "Descripcion de detalle"
    });
    $descripcion.$input.addEventListener('input', function(e){
      e.target.value = e.target.value.toUpperCase();
    });
    let $um = SCSInputFactory('select_search').create({
      type: "select_search", title: "Unidad Medida", size: 4, url: this.url_unidad_medida, value: 'id',
    ...
    let $centro_costo = SCSInputFactory('select_search').create({
      type: "select_search", size: 5, title: "Centro Costo", url: this.url_centro_costo, value: 'id',
    ...
    let $cant = SCSInputFactory('default').create({ type: "number", title: "Cant", size: 2, placeholder: "Cant" });
    let $uuid = SCSInputFactory('hidden').create({type: "hidden"});

    let $button = $create('button');
    $button.classList.add('btn','btn-danger','btn-sm','font-weight-bold', 'mb-4', 'det_delete_button');
    ...
    if(this.opts.solicitud){
      $div.$centro_costo.style.display = 'none';
    } 
  ...
  set_readonly(){
    this.$detalles_button.set_disabled(true);
    // this.$detalles_button.rename("Detalles");
    this.$detalles_button.remove();
    $qs(this.$container, ".det_delete_button").remove();
    super.set_readonly();
  }
