...
async function render_table() {
  ...
  let $edit_buttons = $table.querySelectorAll('.action__edit-button');
  if($edit_buttons) {
    for(let $eb of $edit_buttons){
      $eb.onclick = async function(e) {
        render_form();
        $form.load_to_edit(e.target.closest['action-id'].getAttribute('action-id'));
