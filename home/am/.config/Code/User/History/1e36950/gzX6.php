...
class NotasEntregaFormComponent extends Component
{
    ...
    public function validarDisponible($solicitud){
        ...
        // if(! empty($this->id_solicitudes)){

        //     $union = $this->id_solicitudes;

        // }

        // if(! empty($this->id_solicitud)){

        //     $union[] = $this->id_solicitud;


        // }

        $union = array_merge($this->solicitudes,$this->solicitud);


      //  dd( $solicitud->detalleSolicitudesMaterial);
        ...
        foreach( $solicitud->detalleSolicitudesMaterial as $d){
            ...
            if(! empty($union)) {
                ...
                // array_filter($union, function($solicitudUnion){

                // });

                foreach($union as $solicitudUnion){
                    // dd($union);
                    $detalleSolicitudMaterial = DetalleSolicitudMaterial::where('id_solicitud', $solicitudUnion->id);
