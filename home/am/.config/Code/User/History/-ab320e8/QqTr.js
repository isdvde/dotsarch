...
export class SCSFormRequisicion extends SCSForm {
  ...
  add_detalle(){
    let $detalles = SCSDetalleFactory(this.edit_data.requisicion.tipo).create({solicitud: false});
	...
	async load_to_edit(uuid) {
		...
		if(this.edit_data) {
      // let $detalles = SCSDetalleFactory(this.edit_data.requisicion.tipo).create();
      // this.add_input($detalles);
      this.add_detalle();
