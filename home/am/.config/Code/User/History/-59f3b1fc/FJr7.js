...
function render_form($table, type, create=false) {
  ...
  if(type === "BIENES"){
    ...
    let $fecha = $qs($form, '[name="fecha"]').closest('scs-input').size = 4;
    $fecha.size = 4;
    $fecha.set_size();
    // $qs($form, '[name="fecha"]').closest('scs-input').set_size();

    $qs($form, '[name="prioridad"]').closest('scs-select').size = 4;
    $qs($form, '[name="prioridad"]').closest('scs-select').set_size();
  }
