...
export class TableComponent extends HTMLElement {
	...
	*/
	...
	generate_header_columns(columns, frag = null){
		let $frag = frag || new DocumentFragment();
		let $th = this.$th.cloneNode('true');
		$th.textContent = columns[0];
		if(columns.length !== 1){
			this.generate_header_columns(columns.slice(1), $frag);
		}
		$frag.append($th);
		return $frag;
	}
