	...
	async load_to_edit(uuid) {
      ...
      this.add_input($detalles);
			...
			for(let [key, val] of Object.entries(this.edit_data.requisicion)) {
				...
				let inp = this.querySelector(`input[name=${key}],select[name=${key}],textarea[name=${key}]`);
				if(inp && val){
					try {
...
// let dependencia_solicitante = await axios('/api/compras/unidad-ejecutora').then(res => res.json()).then(res => res.data);
// let dependencia_solicitante = await axios('api/compras/unidad-ejecutora').then(res => res.data.data);
