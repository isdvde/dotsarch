...
import {$, $create, $qs, $qsa, alerter, crudder} from '../libs/utils.js'
import { SCSDropdown } from "./scs-dropdown.js";

export class SCSTable extends HTMLElement {
	...
	*/
	...
	render_table_body() {
		...
		if(this.opts.edit){ this.action_buttons(); }
	}
	...
	action_buttons() {
		...
		for (let [i, $tr] of $trs.entries()) {
			let td = $create('td');
			let $buttons = new SCSDropdown({ title: "" });
			let $edit = $buttons.add_item({ title: "Editar" });
			let $delete = $buttons.add_item({ title: "Eliminar" });
      ...
      $edit.addEventListener('click', async function(e) {
        ...
        let $form = this.edit_action().render_form();
        // await $form.load_to_edit(e.target.closest('[action-id]').getAttribute('action-id'));
        await this.edit_action().$form.load_to_edit(e.target.closest('[action-id]').getAttribute('action-id'));
      }.bind(this));
	...
	async delete_item(e){
		...
		if(confirm){
			try {
				if(uuid) {
					let res = crudder(`${this.base_url.href}/${uuid}`).delete();
					if(res.status === 'error'){
						console.error(res.description.errors);
					} else {
						alerter('Elemento eliminado correctamente').alert();
						await this.refresh();
					}
				}
			} catch (e) {
...
customElements.define('scs-table', SCSTable);