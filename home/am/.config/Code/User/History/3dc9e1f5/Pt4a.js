import { $create, $qsa, alerter, crudder } from "../libs/utils";
...
export class SCSDetalle extends HTMLElement {
  ...
  create_detalle_servicios(opts) {
    ...
    let $centro_costo = new SCSSelectSearch({
      type: "select_search", size: 5, url: this.url_unidad_medida, value: 'id',
      label: ['codigo','descripcion'], placeholder: "Centro Costo"
    });
    ...
    // this.set_internal_value($div, "$descripcion", $descripcion);
    // this.set_internal_value($div, "$um", $um);
    // this.set_internal_value($div, "$cant", $cant);
    // this.set_internal_value($div, "$uuid", $uuid);
