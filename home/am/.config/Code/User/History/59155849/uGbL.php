    ...
    public function index(Request $request)
    {
        ...
        if($query_params){
            ...
            foreach($query_params as $key => $val){
                match($key){
                    'linea_id' => $q->where("lineas.id","=",$val),
                    'unidad_medida_id' => $q->where("unidades_medidas.id","=",$val),
                    'search' => function($key, $val) use($q) {
                        $q->where("articulos.codigo","ilike","%$val%")
                        ->orWhere("articulos.descripcion","ilike","%$val%");
