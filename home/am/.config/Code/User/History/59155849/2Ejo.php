...
class ApiArticuloController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($query_params){
            $q = Articulo::query();
            $q->join('unidades_medidas', 'unidades_medidas.id', '=', 'articulos.unidad_medida_id')
            ...
            foreach($query_params as $key => $val){
                match($key){
                    'linea_id' => $q->join('lineas', 'lineas.id', '=', 'articulos.linea_id')
                    ->select('articulos.*','lineas.descripcion AS linea')
                    ->where("lineas.id","=",$val),
                    'unidad_medida_id' => $q->join('unidades_medidas', 'unidades_medidas.id', '=', 'articulos.unidad_medida_id')
                    ->select('articulos.*','unidades_medidas.descripcion AS unidad_medida','lineas.descripcion AS linea'),
                    default => $q->where('articulos.'.$key, '=', $val),
                };
