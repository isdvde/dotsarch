...
const form_inputs = [
  ...
  { type: "select", title: "Prioridad", name: "prioridad", size: 6, options: {
    text: 'text',
    value: 'text',
    data: prioridad
  }},
  { type: "date", title: "Fecha Recepcion", name: "fecha_recepcion", size: 6},
...
function render_form($table, type, create=false) {
  ...
  if(type === "BIENES"){
    //$qs($form, '[name="codigo_linea_servicio"]').closest('scs-select-search').remove();
    //$form.$inputs = $form.$inputs.filter(el => el.opts.name != "codigo_linea_servicio");
    let $fecha = $qs($form, '[name="fecha"]').closest('scs-input');
    $fecha.size = 4;
    $fecha.set_size();

    let $prioridad = $qs($form, '[name="prioridad"]').closest('scs-select');
    $prioridad.size = 4;
    $prioridad.set_size();

    let $fecha_recepcion = $qs($form, '[name="fecha_recepcion"]').closest('scs-input');
    $fecha_recepcion.size = 4;
    $fecha_recepcion.set_size();
  }
