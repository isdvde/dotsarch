...
export class SCSDetalle extends HTMLElement {
    ...
    this.type_action = {
      'BIENES': function() {
        this.$select = new SCSSelectSearch({
          type: "select_search", title: "Detalles", size: 6, url: this.url_articulo, value: 'uuid',
  ...
  async add_detalle_bienes(opts){
    ...
    // let url = `${this.url_articulo}`;
    let url = new URL(`${basePath}/${this.url_articulo}`);
    if(this.linea) {
      // url = `${url}?linea_id=${this.linea}&`;
      url.searchParams.append('uuid', this.linea);
      console.log('detalle url', url)
      // this.$select.opts.url = url;
      this.$select.url = `${url.pathname}${url.search}`;
      console.log(this.$select.url);
    } else {
      this.$select.opts.url = this.url_articulo;
    }
  }
  ...
  async create_detalle_bienes(art_id, opts) {
    let id = art_id || opts.uuid;
    // let data = await fetch(`${this.url_articulo}/?id=${id}`).then(res => res.json());
    // let url = this.linea ? `${this.url_articulo}?id=${id}&linea_id=${this.linea}` : this.url_articulo;
    // let url = `${this.url_articulo}/${id}`;
    let url = new URL(`${basePath}/${this.url_articulo}`);
    console.log('create detal bienes', url.href)
    // if(this.linea) url = `${url}&linea_id=${this.linea}`;
    // let data = await axios(`${this.url_articulo}/?id=${id}`).then(res => res.data);
    let data = await axios(`${url.href}/${id}`).then(res => res.data.data);
    console.log(data)
    let $div = $create('div');
    ...
    if(opts){
      ...
      $div.$um.$input = data?.unidad_medida;
      $div.$articulo_id.$input = data?.id;
      $div.$um_id.$input = data?.unidad_medida_id;
      $div.$linea_id.$input = data?.linea_id;
    }
