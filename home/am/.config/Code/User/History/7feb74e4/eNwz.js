...
class SCSLinea extends SCSBaseModule {
  ...
  render_form($table, type, create=false) {
    ...
    this.$form.querySelector('[name=tipo]').value = type || '';
    if(create) {
      this.$form.querySelector('option[selected]').removeAttribute('selected');
      this.$form.querySelector('option[value=ACTIVA]').setAttribute('selected', '');
    }
